    ...
    for (let i = 1; i <= paddingDays + daysInMonth; i++) {
        ...
        if (i > paddingDays) {
            ...
            split.forEach((element) => {
                if (
                    ' 39' + monthdisplay + ' ' + daySquare.id + ' ' + yeardisplay + '39' == element ||
                    '39' + monthdisplay + ' ' + daySquare.id + ' ' + yeardisplay + '39' == element
                ) {
            ...
            split.forEach((element) => {
                if (
                    ' 39' + monthdisplay + ' ' + daySquare.id + ' ' + yeardisplay + 's' + '39' == element ||
                    '39' + monthdisplay + ' ' + daySquare.id + ' ' + yeardisplay + 's' + '39' == element
                ) {
            ...
            if (daySquare.id != 'currentDay' && daySquare.id != 'less') {
                function sendUserInfo() {
                    ...
                    request.open(
                        'POST',
                        `/calendar/${JSON.stringify(day_hover)}/${JSON.stringify(monthuser)}/${JSON.stringify(
                            yearuser
                        )}`
                    );
                    request.onload = () => {
                    ...
                    daySquare.addEventListener(
                        'click',
                        () =>
                            (location = `/calendar/${JSON.stringify(day_hover)}/${JSON.stringify(
                                monthuser
                            )}/${JSON.stringify(yearuser)}`)
                    );
                }
                sendUserInfo();
            ...
            if (daySquare.id == 'currentDay') {
                daySquare.addEventListener('click', () => (location = '/today'));
            }
